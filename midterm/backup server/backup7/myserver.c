...
int main(int argc, char const *argv[])
{
	...
	while(1){
		...
		/* Get server time */
	   	...
	   	sentData1.connection_time = (start.tv_sec*THOUSAND + start.tv_nsec/MILLION);	
		...
		if(pid == 0){			
			...
			while(time_of_round--)
			{
				if(die_flag){
					...
					fprintf(logfile, "Childs notified and killed\n");
					fflush(logfile);
					break;
				}
