...
int main(int argc, char const *argv[])
{
    ...
    if(IP_OF_MULT_SERV == NULL)
        mult_addr.sin_addr.s_addr = htonl(INADDR_ANY); //inet_addr(ipAdress);
    else
        mult_addr.sin_addr.s_addr = inet_addr(IP_OF_MULT_SERV);
    ...
    elapsed = (finish.tv_sec*MILLION + finish.tv_nsec/1000) -(begin.tv_sec*MILLION + begin.tv_nsec/1000);

    printf("\nELAPSED TIME: %d.%.2f sec\n", (int)elapsed/MILLION, (float)elapsed/1000  );
    printf("Number of invertable Matrices: %d\n", verified);
    printf("Number of non-invertable Matrices %d\n", non_verified);
...
void verifier(void* arg){
    ...
    while(!die_flag){
        ...
        if(DEBUG)
            printf("\nVerifier thread: %u in critical section in verifier module \n\n", (unsigned int) pthread_self());
        // printf("size: %d\n", size);
        /*
...
int get_from_module1(){
    ...
    if(size > 0){
        if(DEBUG)
            printf("Original A from module1\n");
        for(i = 0; i < size; ++i){
        ...
        if(DEBUG)    
            printf("Inverted A from module1\n");
        for(i = 0; i < size; ++i){
...
void getback(int size){
    int i, j;

    printf("\nMultiplication of A and inversed A: \n");

    for(i = 0; i < size; ++i){
        recv(mult_sock, matrixA[i], size*sizeof(float), 0);
        if(DEBUG){
            for(j = 0; j < size; ++j){
                printf("%6.5f ", matrixA[i][j]);
            }
            printf("\n");    
        }
    }
...
void print_usage(){
    puts("Usage: ./module3 <[L] QUEUE_SIZE> <[P] NUM_OF_THREAD> <DEBUG MODE> <IP_OF_MULT_SERVER>");
    puts("Example: ./module1 200 2 1 192.168.?.?");
}
