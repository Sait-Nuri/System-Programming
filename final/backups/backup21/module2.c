...
void multiplier(void* arg){
    ...
    while(!die_flag){
        ...
        if(DEBUG)
            printf("thread: %u in critical section in multiplier module\n", (unsigned int) pthread_self());
...
void print_usage(){
    puts("Usage: ./module2 <[L] QUEUE_SIZE> <[P] NUM_OF_THREAD> <DEBUG MODE>");
