...
matrix_queue_t *matrix_buffer;
LU_queue_t * lu_buffer;
LU_queue_t * invd_lu_buffer;
...
int main(int argc, char const *argv[]){
    ...
    /*****************************************************************************/
	if(DEBUG)
		printf("\n------------ DEBUGGING MOD ACTIVATED ----------\n");

	

	matrix_buffer = calloc(QUEUE_SIZE, sizeof(matrix_queue_t));
	lu_buffer = calloc(QUEUE_SIZE, sizeof(LU_queue_t));
	invd_lu_buffer = calloc(QUEUE_SIZE, sizeof(LU_queue_t));
	...
	printf("NUMBER OF MATRICES PRODUCED %d\n", MAX_PRODUCE);
	printf("NUMBER OF MATRICES DECOMPOSED TO L.U %d\n", MAX_PRODUCE);
	printf("NUMBER OF MATRICES INVERTED %d \n", MAX_PRODUCE);
	printf("ELAPSED TIME: %d.%.2f sec\n", (int)elapsed/MILLION, (float)elapsed/1000  );
...
void produce_matrix(void* size){
	...
	while(!die_flag){
		...
		if(DEBUG)
			printf("\nthread: %u in critical section in producer module\n", (unsigned int) pthread_self());
		//printf("flag7\n");
		...
		if(DEBUG)
			printf("\nqueue index in produser module %d\n", mbuf_index);
		...
		for(i = 0; i < MATRIX_SIZE; ++i){
			for (j = 0; j < MATRIX_SIZE; ++j){								
				matrix_buffer[mbuf_index].m[i][j] = abs((rand())%50+1);
			}
		}
...
void decompose(void* arg){
	...
	while(!die_flag){
		...
		if(DEBUG)
			printf("\nthread: %u in critical section in decomposition module\n", (unsigned int) pthread_self());
		...
		// printf("bayrak1\n");
		if(DEBUG)
			printf("\nqueue index in decomposition module %d\n", lu_buf_index);
		// sleep(1);
